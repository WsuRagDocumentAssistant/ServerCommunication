...
class GeminiService(BaseLLMApiInterface):
    ...
    def _config(self, max_tokens: int, temperature: Optional[float], response_format: Optional[dict]):
        kwargs = {}
    ...
    async def chat(
        ...
        response = await self._client.aio.models.generate_content(
            model=_model, contents=prompt, config=self._config(max_tokens, temperature, response_format),
        )
